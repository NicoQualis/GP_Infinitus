...
public class TC_63_GB_PASOS_ADQ_700 {
	...
	public void pagina3(Reports report, Repo_WebRepository repo, DriverManager DM, String name) {
		...
		repo.get_obj_inputById("parametro_03").sendKeys("8228");
		...
		try {
			...
			if (report.validateObjectIsDisplayable(repo.get_obj_lblRunIdRan(runId))) {
				...
				if (chkProcesado.equals("1/0")) {
					report.AddLine("Resultado Correcto. Se procesaron 1 Registro OK y 0 Errores");
					System.out.println("Resultado Correcto. Se procesaron 1 Registro OK y 0 Errores");
					Status[0] = "P";
				} else {
