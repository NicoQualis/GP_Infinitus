...
public class TC_81_GB_PASOS_ADQ_700 {
	private WebDriverWait waitFor;

	public void pagina3(Reports report, Repo_WebRepository repo, DriverManager DM, String name) {

		report.AddLine("se ingresa identificador fecha de cierre");
		System.out.println("se ingresa identificador fecha de cierre");
		repo.get_obj_inputById("parametro_01").clear();
		repo.get_obj_inputById("parametro_01").sendKeys("ASDFASDFASDF");

		report.AddLine("Se ingresa el identificador comercio");
		System.out.println("Se ingresa el identificador comercio");
		...
		try {
			...
			if (report.validateObjectIsDisplayable(repo.get_obj_lblRunIdRan(runId))) {
				...
				if (chkProcesado.equals("0/0")) {
					report.AddLine("Resultado Correcto. Se procesaron 0 Registros OK y 0 Errores");
					System.out.println("Resultado Correcto. Se procesaron 0 Registros OK y 0 Errores");
					Status[0] = "P";
				} else {
					report.AddLineAssertionError("Error - Se procesaron " + chkProcesado.split("/")[0] + " de 0 Registros y " + chkProcesado.split("/")[1] + " de 0 Errores");
					System.out.println("Error - Se procesaron " + chkProcesado.split("/")[0] + " de 0 Registros y " + chkProcesado.split("/")[1] + " de 0 Errores");
					Status[0] = "Fail - Se procesaron " + chkProcesado.split("/")[0] + " de 0 Registros y " + chkProcesado.split("/")[1] + " de 0 Errores";
				}
				...
				case "background: rgb(252, 77, 77)":
					report.AddLine("Resultado correcto. El color de fondo es Rojo");
					System.out.println("Resultado correcto. El color de fondo es Rojo");
					Status[1] = "P";
					break;
				default:
