...
public class TC_60_GB_PASOS_ADQ_700 {
	...
	public void pagina3(Reports report, Repo_WebRepository repo, DriverManager DM, String name) {
		...
		DateTimeFormatter yearSystemDTF = DateTimeFormatter.ofPattern("yyyyMMdd");
		String sysDate = yearSystemDTF.format(LocalDateTime.now().plusDays(1)); 
		...
		repo.get_obj_inputById("parametro_01").sendKeys("8228");
		...
		repo.get_obj_inputById("parametro_02").sendKeys(sysDate);
	...
	public String[] ValidacionesWeb (Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId) {
		...
		WebDriverWait wait = new WebDriverWait(DM.getActualDriver(), 60);
