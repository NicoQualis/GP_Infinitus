package Pasos.GlobalBatch_ADQ_700;
...
public class TC_80_GB_PASOS_ADQ_700 {
	private WebDriverWait waitFor;

	public void pagina3(Reports report, Repo_WebRepository repo, DriverManager DM, String name) {

		report.AddLine("se ingresa la fecha de cierre");
		System.out.println("se ingresa la fecha de cierre");
		repo.get_obj_inputById("parametro_01").clear();
		repo.get_obj_inputById("parametro_01").sendKeys("20220804");

		report.AddLine("Se ingresa el comercio");
		System.out.println("Se ingresa el comercio");
		repo.get_obj_inputById("parametro_02").clear();
		repo.get_obj_inputById("parametro_02").sendKeys("8228");
	...
	public String[] ValidacionesWeb (Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId) {
		...
		WebDriverWait wait = new WebDriverWait(DM.getActualDriver(), 120);
		...
		try {
			...
			if (report.validateObjectIsDisplayable(repo.get_obj_lblRunIdRan(runId))) {
				...
				if (chkProcesado.equals("0/0")) {
					report.AddLine("Resultado Correcto. Se procesó 0 Registro OK y 0 Errores");
					System.out.println("Resultado Correcto. Se procesó 0 Registro OK y 0 Errores");
					Status[0] = "P";
				} else {
					report.AddLineAssertionError("Error - Se procesaron " + chkProcesado.split("/")[0] + " de 0 Registro y " + chkProcesado.split("/")[1] + " de 0 Errores");
					System.out.println("Error - Se procesaron " + chkProcesado.split("/")[0] + " de 0 Registro y " + chkProcesado.split("/")[1] + " de 0 Errores");
					Status[0] = "Fail - Se procesaron " + chkProcesado.split("/")[0] + " de 0 Registro y " + chkProcesado.split("/")[1] + " de 0 Errores";
				}
				
				switch(chkColor.split(";")[0]) {
				case "background: rgb(181, 235, 117)":
					report.AddLineAssertionError("Error - El color de fondo es Verde");
					System.out.println("Error - El color de fondo es Verde");
					Status[1] = "Fail - Color de fondo es Verde";
					break;
				case "background: rgb(252, 168, 77)":
					report.AddLine("Resultado Correcto. El color de fondo es Naranja");
					System.out.println("Resultado Correcto. El color de fondo es Naranja");
					Status[1] = "P";
					break;
