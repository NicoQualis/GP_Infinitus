...
public class TC_64_GB_PASOS_ADQ_700 {
	...
	public void pagina3(Reports report, Repo_WebRepository repo, DriverManager DM, String name) {
		...
		repo.get_obj_inputById("parametro_02").sendKeys("20220728");
		...
		repo.get_obj_inputById("parametro_03").sendKeys("ASDFASDFASDF");
	...
	public String[] ValidacionesWeb (Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId) {
		...
		try {
			...
			if (report.validateObjectIsDisplayable(repo.get_obj_lblRunIdRan(runId))) {
				...
				if (chkProcesado.equals("2/0")) {
					report.AddLine("Resultado Correcto. Se procesaron 2 Registros OK y 0 Errores");
					System.out.println("Resultado Correcto. Se procesaron 2 Registros OK y 0 Errores");
					Status[0] = "P";
				} else {
					report.AddLineAssertionError("Error - Se procesaron " + chkProcesado.split("/")[0] + " de 2 Registros y " + chkProcesado.split("/")[1] + " de 0 Errores");
					System.out.println("Error - Se procesaron " + chkProcesado.split("/")[0] + " de 2 Registros y " + chkProcesado.split("/")[1] + " de 0 Errores");
					Status[0] = "Fail - Se procesaron " + chkProcesado.split("/")[0] + " de 2 Registros y " + chkProcesado.split("/")[1] + " de 0 Errores";
				}
