...
public class TC_07_AUT_API_VISA{
	...
	public boolean Test(Reports report, String name, String configEntidad) {
		...
		try {	
			...
			String path = "./API_Requests/Autorizador/VISA/TC_60_Crédito_AnulaciónLocal_Banda.txt";
			String pathPre = "./API_Requests/Autorizador/VISA/PRE_BANDA_VISA.txt";
			...
			//Cambio de simulador a reverso
			System.out.println("##### INICIO CAMBIO DE SIMULADOR PARA REVERSO #####");
			report.AddLine("##### INICIO CAMBIO DE SIMULADOR PARA REVERSO#####");

			apiCmd.stopOpiCmd(report, configEntidad, "opi4qa");
			Thread.sleep(2000);
			apiCmd.cambioSimuladorVisa(report, configEntidad, "opi4qa", "700", "reversos");
			Thread.sleep(2000);
			apiCmd.startOpiCmd(report, configEntidad, "opi4qa");
			Thread.sleep(10000);
			
			System.out.println("##### FIN CAMBIO DE SIMULADOR PARA REVERSO #####");
			report.AddLine("##### FIN CAMBIO DE SIMULADOR PARA REVERSO #####");
...
//			token = AI.getToken(report, apiResp);
			...
			AI.setEndpoint(report, repoVar, "http://10.72.0.22:8293/Void");
			...
			if (!bodyData.isEmpty() && !token.isEmpty())
			{
				...
				if (responseCodeData.equals("00")) {
					...
					report.AddLine("FAIL<br>responseCode: " + responseCodeData + " Se esperaba: 00");
				...
				apiCmd.cambioSimuladorVisa(report, configEntidad, "opi4qa", "700", "compras");
	...
	private boolean validacionGral(dbWorker oraResp, Reports report, String rrnLast6) throws SQLException {
		...
		//Primera Validación
		queryVerf = "select id_estado, modo_ingreso, id_autorizacion, id_autorizacion_adquirente from autorizacion where id_autorizacion_adquirente = " + rrnLast6;

		primeraValidacion(oraResp, report, queryVerf);
		
		//Segunda Validación
		queryVerf = "select anular_reversar, COALESCE(id_auto_adq_anular_reversar, (-1)) from autorizacion_adquirente_log where id_autorizacion_adquirente = " + rrnLast6;

		segundaValidacion(oraResp, report, queryVerf);
		
		//Tercera Validación
		queryVerf = "select  in_cod_procesamiento, in_modo_ingreso_terminal, id_tipo_autorizacion_adq_log, out_tipo_mensaje, id_autorizacion_original, id_auto_adq_origen from autorizacion_adquirente_log where id_autorizacion_adquirente in (select id_auto_adq_anular_reversar from autorizacion_adquirente_log where id_autorizacion_adquirente = " + rrnLast6 + ")";

		tercerValidacion(oraResp, report, queryVerf);
					
		//Cuarta Validación
		queryVerf = "select in_tipo_mensaje from respuesta_mc_log where id_autorizacion_adquirente in (select id_auto_adq_anular_reversar from autorizacion_adquirente_log where id_autorizacion_adquirente = " + rrnLast6 + ")";
	...
	private void primeraValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Validación del MODO_INGRESO - Tiene que ser igual a 90
		if (!respVerifBDD[1].equals("90")) {
			report.AddLine("MODO_INGRESO: " + respVerifBDD[1] + ". Se esperaba: 90");
			System.out.println("MODO_INGRESO: " + respVerifBDD[1] + ". Se esperaba: 90\r\n");
	...
	private void tercerValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Validación del IN_MODO_INGRESO_TERMINAL - Tiene que ser: '9010'
		if (!respVerifBDD[1].equals("9010")) {
			report.AddLine("IN_MODO_INGRESO_TERMINAL: " + respVerifBDD[1] + ". Se esperaba: 9010");
			System.out.println("IN_MODO_INGRESO_TERMINAL: " + respVerifBDD[1] + ". Se esperaba: 9010\r\n");
			Status[5] = "FAIL - Validación del IN_MODO_INGRESO_TERMINAL";
		}	else {
			report.AddLine("Resultado Correcto. IN_MODO_INGRESO_TERMINAL: " + respVerifBDD[1]);
			System.out.println("Resultado Correcto. IN_MODO_INGRESO_TERMINAL: " + respVerifBDD[1] + "\r\n");
			Status[5] = "P";
		}

		//Validación del ID_TIPO_AUTORIZACION_ADQ_LOG - Tieme qie ser: '6'
		if (!respVerifBDD[2].equals("6")) {
			report.AddLine("ID_TIPO_AUTORIZACION_ADQ_LOG: " + respVerifBDD[2] + ". Se esperaba: 6");
			System.out.println("ID_TIPO_AUTORIZACION_ADQ_LOG: " + respVerifBDD[2] + ". Se esperaba: 6\r\n");
