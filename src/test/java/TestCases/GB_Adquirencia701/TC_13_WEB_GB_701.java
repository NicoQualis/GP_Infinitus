...
import Pasos.GlobalBatch_ADQ_701.TC_13_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_13_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_13_GB_PASOS_ADQ_701 pasos = new TC_13_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_13_GB_PASOS_ADQ_701 pasos) {
...
;
		...
		queryVerif ="select c.tipo_producto, c.fecha_pago_comercio\r\n"
				...
				+ "where a.id_autorizacion_adquirente = '19250'";
		...
		//Validacion 02
		if (rta[1].equals("2022-06-09 00:00:00")) {
			...
			report.AddLineAssertionError("El campo FECHA_PAGO_COMERCIO tiene el registro: "+rta[1]+", se esperaba el registro 2022-06-09 00:00:00");
			System.out.println("El campo FECHA_PAGO_COMERCIO tiene el registro: "+rta[1]+", se esperaba el registro 2022-06-09 00:00:00 \r\n");
	...
	private boolean execPreCondiciones(Reports report, dbWorker oraWorker) {
		...
		//--UPDATE--
		//Acción 01 - AUTORIZACION_ADQUIRIENTE_LOG
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 02	
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '19250'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19250'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19250'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19250'))";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 06	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '19250'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	
		//Accion 07	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19250'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 08
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19250'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		oraWorker.getDisConn(conn);
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '19250'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19250'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19250'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19250'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '19250'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19250'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19250'", conn);
