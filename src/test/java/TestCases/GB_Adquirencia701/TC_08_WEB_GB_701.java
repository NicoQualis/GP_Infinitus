...
import Pasos.GlobalBatch_ADQ_701.TC_08_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_08_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name,
		...
		try {
			...
			TC_08_GB_PASOS_ADQ_701 pasos = new TC_08_GB_PASOS_ADQ_701();
			...
			pasos.pagina3(report, repo, DM, name); // Botón Lanzar
			pasos.pagina4(report, repo, DM, name); // Botón Confirmar
			runId = pasos.pagina5(report, repo, DM, name); // runID
	...
	private boolean execPreCondiciones(Reports report, dbWorker oraWorker) {
		...
		//Accion 02 - TABLA AUTORIZACION ADQUIRENTE_LOG
		...
		//Accion 03 - TABLA PRESENTACIONES
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name,
			String runId, TC_08_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		if (rta.equals("2022-06-09 00:00:00")) {
			...
			System.out.println("El campo FECHA_PAGO_COMERCIO tiene el registro: "+rta+", se esperaba el registro 2022-06-09 00:00:00 \r\n");
	...
	private boolean execPostCondiciones(Reports report, dbWorker oraWorker) {
		...
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '19244'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		// ACCION 02 - TABLA CONSUMOS_CUOTAS
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19244'))";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		// ACCION 03 - TABLA CONSUMOS_DATOS_ADICIONALES
		queryPost =  "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19244'))";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		// ACCION 04- TABLA CONSUMOS
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19244'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		// ACCION 05 - TABLA AUTORIZACION
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '19244'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		// ACCION 06 - TABLA RESPUESTA_MC_LOG
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19244'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		// ACCION 07 - TABLA AUTORIZACION_ADQUIRENTE_LOG
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19244'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '19244'");

		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19244'))");

		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19244'))");

		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19244'))");

		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '19244'");
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19244'");
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19244'");
