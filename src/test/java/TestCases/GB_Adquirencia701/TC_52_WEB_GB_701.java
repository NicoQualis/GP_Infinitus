...
import Pasos.GlobalBatch_ADQ_701.TC_52_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_52_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_52_GB_PASOS_ADQ_701 pasos = new TC_52_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_52_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		conn = oraWorker.getConn();
		//Accion 01		
		queryPost = "update autorizacion_adquirente_log set id_auto_adq_origen = null, id_autorizacion_original = null where id_autorizacion_adquirente in ('21640')";
		res.add(oraWorker.oraUpdate(queryPost, conn));

		//Accion 02	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE in ('21639', '21640')";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE in ('21639', '21640')";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE in ('21639', '21640')";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		//Accion 01		
		oraWorker.oraUpdate("update autorizacion_adquirente_log set id_auto_adq_origen = null, id_autorizacion_original = null where id_autorizacion_adquirente in ('21640')", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE in ('21639', '21640')", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE in ('21639', '21640')", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE in ('21639', '21640')", conn);
