...
import Pasos.GlobalBatch_ADQ_701.TC_42_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_42_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_42_GB_PASOS_ADQ_701 pasos = new TC_42_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_42_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif ="select c.id_mod_financ_comercio, c.importe_interes_com, c.porcentaje_financ, c.id_sistema_financiacion, c.tipo_porc_financ, c.importe_iva_interes_com\r\n"
				...
				+ "where a.id_autorizacion_adquirente in ('021418')";
		...
		//Validacion 01 - ID_MOD_FINANC_COMERCIO		
		if (rta[0].equals("15")) {
			report.AddLine("Resultado correcto. La tabla ID_MOD_FINANC_COMERCIO tiene el registro: "+rta[0]);
			System.out.println("Resultado correcto. La tabla ID_MOD_FINANC_COMERCIO tiene el registro: "+rta[0]);
			Status[0] = "P";
		} else {
			report.AddLineAssertionError("ERR - La tabla ID_MOD_FINANC_COMERCIO tiene el registro: "+rta[0]+", se esperaba el registro 15");
			System.out.println("ERR - La tabla ID_MOD_FINANC_COMERCIO  tiene el registro: "+rta[0]+", se esperaba el registro 15");
			Status[0] = "FAIL - ID_MOD_FINANC_COMERCIO";
		}

		//Validacion 02 - IMPORTE_INTERES_COM
		if (rta[1].equals("-434.05")) {
			report.AddLine("Resultado correcto. La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]);
			System.out.println("Resultado correcto. La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]);
			Status[1] = "P";
		} else {
			report.AddLineAssertionError("ERR - La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -434.05");
			System.out.println("ERR - La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -434.05");
			Status[1] = "FAIL - IMPORTE_INTERES_COM";
		}
				
		//Validacion 03 - PORCENTAJE_FINANC 	
		if (rta[2].equals("10")) {
			report.AddLine("Resultado correcto. La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]);
			System.out.println("Resultado correcto. La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]);
			Status[2] = "P";
		} else {
			report.AddLineAssertionError("ERR - La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]+", se esperaba el registro 10");
			System.out.println("ERR - La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]+", se esperaba el registro 10");
			Status[2] = "FAIL - PORCENTAJE_FINANC";
		}
						
		//Validacion 04 - ID_SISTEMA_FINANCIACION
		if (rta[3].equals("7")) {
			report.AddLine("Resultado correcto. La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]);
			System.out.println("Resultado correcto. La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]);
			Status[3] = "P";
		} else {
			report.AddLineAssertionError("ERR - La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]+", se esperaba el registro 7");
			System.out.println("ERR - La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]+", se esperaba el registro 7");
			Status[3] = "FAIL - ID_SISTEMA_FINANCIACION";
		}
				
		//Validacion 05	- TIPO_PORC_FINANC
		if (rta[4].equals("2")) {
			report.AddLine("Resultado correcto. La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]);
			System.out.println("Resultado correcto. La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]);
			Status[4] = "P";
		} else {
			report.AddLineAssertionError("ERR - La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]+", se esperaba el registro 2");
			System.out.println("ERR - La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]+", se esperaba el registro 2");
			Status[4] = "FAIL - TIPO_PORC_FINANC";
		}
						
		//Validacion 06	- IMPORTE_IVA_INTERES_COM
		if (rta[5].equals("-95.49")) {
			report.AddLine("Resultado correcto. La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]);
			System.out.println("Resultado correcto. La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]);
			Status[5] = "P";
		} else {
			report.AddLineAssertionError("ERR - La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -95.49");
			System.out.println("ERR - La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -95.49");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 01		
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '021418'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 02	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021418'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021418'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021418'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '021418'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 06	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '021418'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 07	
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '021418'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '021418'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021418'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021418'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021418'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '021418'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '021418'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '021418'", conn);
