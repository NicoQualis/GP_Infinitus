...
import Pasos.GlobalBatch_ADQ_701.TC_44_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_44_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		TC_44_GB_PASOS_ADQ_701 pasos = new TC_44_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_44_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		String[] rta1 = new String[2];
		String[] rta2 = new String[2];
		String[] rta3 = new String[2];
		String queryVerif;

		System.out.println("##### INICIO EJECUCION DE VALIDACIONES #####");
		report.AddLine("##### INICIO EJECUCION DE VALIDACIONES #####");

		queryVerif = "select cc.nro_cuota, cc.fecha_pago_comercio "
				+ "from consumos c "
				...
				+ "where a.id_autorizacion_adquirente in ('21545') and cc.nro_cuota = '1'";
		
		rta1 = oraWorker.oraTwoQuery(queryVerif);
		//Continua a pesar que el valor de la tabla sea null
		for (int i = 0; i<6; i++) {
				if (rta1[i] == null)
					rta1[i] = "null";
			}		
		//Validacion 01	
		if (rta1[0].contentEquals("1")) {
			report.AddLine("Resultado Correcto. El campo nro_cuota tiene el registro: " + rta1[0]);
			System.out.println("Resultado Correcto. El campo nro_cuota tiene el registro:" + rta1[0] );
			Status[0] = "P";
		} else {
			report.AddLineAssertionError("El campo nro_cuota tiene el registro:" + rta1[0] + ", Se esperaba el registro 1");
			System.out.println("El campo nro_cuota tiene el registro:" + rta1[0] + ", Se esperaba el registro 1");
			Status[0] = "FAIL - nro_cuota";
		}
		
		//Validacion 02
		if (rta1[1].equals("2022-08-03 00:00:00")) {
			report.AddLine("Resultado Correcto. El campo fecha_pago_comercio tiene el registro: " + rta1[1]+".");
			System.out.println("Resultado Correcto. El campo fecha_pago_comercio tiene el registro: " + rta1[1]+".");
			Status[1] = "P";
		} else {
			report.AddLineAssertionError("El campo fecha_pago_comercio tiene el registro: "+rta1[1]+", se esperaba el registro 2022-08-03 00:00:00");
			System.out.println("El campo fecha_pago_comercio tiene el registro: "+rta1[1]+", se esperaba el registro 2022-08-03 00:00:00");
		...
		queryVerif ="select cc.nro_cuota, cc.fecha_pago_comercio "
				...
				+ "inner join autorizacion a on c.id_autorizacion = a.id_autorizacion "
				+ "inner join autorizacion_adquirente_log aal on a.id_autorizacion_adquirente = aal.id_autorizacion_adquirente "
				+ "where a.id_autorizacion_adquirente in ('21545') and cc.nro_cuota = '2'";
		
		rta2 = oraWorker.oraTwoQuery(queryVerif);
		queryVerif="";
				
		//Validacion 03
		if (rta2[0].contentEquals("2")) {
			report.AddLine("Resultado Correcto. El campo nro_cuota tiene el registro: " + rta2[0]);
			System.out.println("Resultado Correcto. El campo porcentaje_financ tiene el registro: " + rta2[0]);
			Status[2] = "P";
		} else {
			report.AddLineAssertionError("El campo nro_cuota tiene el registro: "+rta2[0]+", se esperaba el registro 2");
			System.out.println("El campo nro_cuota tiene el registro: "+rta2[0]+", se esperaba el registro 2");
			Status[2] = "FAIL - nro_cuota";
		}		
		
		//Validacion 04
		if (rta2[1].equals("2022-08-03 00:00:00")) {
			report.AddLine("Resultado Correcto. El campo fecha_pago_comercio tiene el registro: " + rta2[1]);
			System.out.println("Resultado Correcto. El campo fecha_pago_comercio tiene el registro: " + rta2[1]);
			Status[3] = "P";
		} else {
			report.AddLineAssertionError("El campo fecha_pago_comercio tiene el registro: "+rta2[1]+", se esperaba el registro 2022-08-03 00:00:00");
			System.out.println("El campo fecha_pago_comercio tiene el registro: "+rta2[1]+", se esperaba el registro 2022-08-03 00:00:00");
			Status[3] = "FAIL - fecha_pago_comercio";
		}
		...
		queryVerif ="select cc.nro_cuota, cc.fecha_pago_comercio "
				...
				+ "inner join autorizacion a on c.id_autorizacion = a.id_autorizacion "
				+ "inner join autorizacion_adquirente_log aal on a.id_autorizacion_adquirente = aal.id_autorizacion_adquirente "
				+ "where a.id_autorizacion_adquirente in ('21545') and cc.nro_cuota = '3'";
		
		rta3 = oraWorker.oraTwoQuery(queryVerif);
		queryVerif="";
		
		//Validacion 05		
		if (rta3[0].contentEquals("3")) {
			report.AddLine("Resultado Correcto. El campo nro_cuota tiene el registro: " + rta3[0]);
			System.out.println("Resultado Correcto. El campo nro_cuota tiene el registro: " + rta3[0]);
			Status[4] = "P";
		} else {
			report.AddLineAssertionError("El campo nro_cuota tiene el registro: "+rta3[0]+", se esperaba el registro 3");
			System.out.println("El campo nro_cuota tiene el registro: "+rta3[0]+", se esperaba el registro 3");
			Status[4] = "FAIL - tipo_porc_financ";
		}
		
		//Validacion 06
		if (rta3[1].equals("2022-08-03 00:00:00")) {
			report.AddLine("Resultado Correcto. El campo fecha_pago_comercio tiene el registro: " + rta3[1]);
			System.out.println("Resultado Correcto. El campo fecha_pago_comercio tiene el registro: " + rta3[1]);
			Status[5] = "P";
		} else {
			report.AddLineAssertionError("El campo fecha_pago_comercio tiene el registro: "+rta3[1]+", se esperaba el registro 2022-08-03 00:00:00");
			System.out.println("El campo fecha_pago_comercio tiene el registro: "+rta3[1]+", se esperaba el registro 2022-08-03 00:00:00");
			Status[5] = "FAIL - fecha_pago_comercio";
		}
	...
	private boolean execPreCondiciones(Reports report, dbWorker oraWorker) {
		...
		for(Integer resultados : res) {
			if (resultados < 1) {
				index ++;
				report.AddLine("===== Error en la acción: " + index + " =====");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '21545'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 02	- TABLA CONSUMOS_CUOTAS
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21545'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	- TABLA CONSUMOS_DATOS_ADICIONALES
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21545'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	- TABLA CONSUMOS
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21545'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	- TABLA AUTORIZACION
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '21545'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 06	- TABLA RESPUESTA_MC_LOG
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21545'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	
		//Accion 07	- TABLA AUTORIZACION_ADQUIRENTE_LOG
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21545'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '21545'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21545'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21545'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21545'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '21545'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21545'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21545'", conn);
