...
import Pasos.GlobalBatch_ADQ_701.TC_34_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_34_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_34_GB_PASOS_ADQ_701 pasos = new TC_34_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_34_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif ="select c.id_mod_financ_comercio, c.importe_interes_com, c.porcentaje_financ, c.id_sistema_financiacion, c.tipo_porc_financ, c.importe_iva_interes_com\r\n"
				...
				+ "where a.id_autorizacion_adquirente in ('21545')";
		...
		if (rta[0].equals("12")) {
			...
			report.AddLineAssertionError("La tabla ID_MOD_FINANC_COMERCIO tiene el registro: "+rta[0]+", se esperaba el registro 12");
			System.out.println("La tabla ID_MOD_FINANC_COMERCIO  tiene el registro: "+rta[0]+", se esperaba el registro 12");
			Status[0] = "FAIL - ID_MOD_FINANC_COMERCIO";
		}

		//Validacion 02		
		if (rta[1].equals("-3600")) {
			...
			report.AddLineAssertionError("La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -3600");
			System.out.println("La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -3600");
		...
		if (rta[2].equals("20")) {
			...
			report.AddLineAssertionError("La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]+", se esperaba el registro 20");
			System.out.println("La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]+", se esperaba el registro 20");
		...
		if (rta[3].equals("2")) {
			...
			report.AddLineAssertionError("La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]+", se esperaba el registro 2");
			System.out.println("La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]+", se esperaba el registro 2");
		...
		if (rta[4].equals("1")) {
			...
			report.AddLineAssertionError("La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]+", se esperaba el registro 1");
			System.out.println("La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]+", se esperaba el registro 1");
		...
		if (rta[5].equals("-792")) {
			...
			report.AddLineAssertionError("La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -792");
			System.out.println("La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -792");
