...
import Pasos.GlobalBatch_ADQ_701.TC_07_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_07_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		TC_07_GB_PASOS_ADQ_701 pasos = new TC_07_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_07_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif = "select c.tipo_producto\r\n"
				...
				+ "where a.id_autorizacion_adquirente = '19242'";
		...
		if (rta.equals("2022-06-05 00:00:00")) {
			...
			report.AddLineAssertionError("El campo fecha_pago_comercio tiene el registro: "+rta+", se esperaba el registro 2022-06-05 00:00:00");
			System.out.println("El campo fecha_pago_comercio tiene el registro: "+rta+", se esperaba el registro 2022-06-05 00:00:00");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '19242'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 02 - TABLA CONSUMOS_CUOTAS
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19242'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03 - TABLA CONSUMOS_DATOS_ADICIONALES
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19242'))";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 04 - TABLA CONSUMOS
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19242'))";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 05 - TABLA AUTORIZACION
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '19242'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 06 - TABLA RESPUESTA_MC_LOG
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19242'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 07 - TABLA AUTORIZACION_ADQUIRENTE_LOG
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19242'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '19242'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19242'))",conn);		
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19242'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '19242'))",conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '19242'",conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19242'",conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '19242'",conn);
