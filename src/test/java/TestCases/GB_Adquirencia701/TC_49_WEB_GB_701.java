...
import Pasos.GlobalBatch_ADQ_701.TC_49_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_49_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_49_GB_PASOS_ADQ_701 pasos = new TC_49_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_49_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif ="select clc.cant_consumos, clc.importes_consumos, clc.cant_ajustes, clc.importes_ajustes \r\n"
				...
				+ "where clc.id_comercio = '8228' \r\n"
				+ "and clc.fecha_cierre = '21/07/2022' \r\n"
				+ "and clc.fecha_pago = '26/07/2022' \r\n"
				+ "and clc.id_moneda = '858'  \r\n"
				+ "and clc.id_marca = '2'";

		rta = oraWorker.oraFourQuery(queryVerif);

		//Validación del CANT_CONSUMOS - Tiene que ser igual a 1
		if (!rta[0].equals("1")) {
			report.AddLineAssertionError("El campo CANT_CONSUMOS tiene el registro: " + rta[0].toString() + ". Se esperaba: 1");
			System.out.println("El campo CANT_CONSUMOS tiene el registro: " + rta[0].toString() + ". Se esperaba: 1\r\n");
			Status[0] = "FAIL - Validación del CANT_CONSUMOS";
		}	else {
			report.AddLine("Resultado Correcto. El campo CANT_CONSUMOS tiene el registro: " + rta[0].toString());
			System.out.println("Resultado Correcto. El campo CANT_CONSUMOS tiene el registro: " + rta[0] + "\r\n");
			Status[0] = "P";
		}
		
		//Validación del IMPORTES_CONSUMOS  - Tiene que ser igual a 30000
		if (!rta[1].equals("30000")) {
			report.AddLineAssertionError("El campo IMPORTES_CONSUMOS  tiene el registro: " + rta[1].toString() + ". Se esperaba: 30000");
			System.out.println("El campo IMPORTES_CONSUMOS  tiene el registro: " + rta[1].toString() + ". Se esperaba: 30000 \r\n");
			Status[1] = "FAIL - Validación del IMPORTES_CONSUMOS ";
		}	else {
			report.AddLine("Resultado Correcto. El campo IMPORTES_CONSUMOS  tiene el registro: " + rta[1].toString());
			System.out.println("Resultado Correcto. El campo IMPORTES_CONSUMOS  tiene el registro: " + rta[1] + "\r\n");
			Status[1] = "P";
		}
		
		//Validación del CANT_AJUSTES   - Tiene que ser igual a 0
		if (!rta[2].equals("0")) {
			report.AddLineAssertionError("El campo CANT_AJUSTES   tiene el registro: " + rta[2].toString() + ". Se esperaba: 0");
			System.out.println("El campo CANT_AJUSTES   tiene el registro: " + rta[2].toString() + ". Se esperaba: 0\r\n");
			Status[2] = "FAIL - Validación del CANT_AJUSTES  ";
		}	else {
			report.AddLine("Resultado Correcto. El campo CANT_AJUSTES   tiene el registro: " + rta[2].toString());
			System.out.println("Resultado Correcto. El campo CANT_AJUSTES   tiene el registro: " + rta[2] + "\r\n");
			Status[2] = "P";
		}
		
		//Validación del IMPORTES_AJUSTES    - Tiene que ser igual a 0
		if (!rta[3].equals("0")) {
			report.AddLineAssertionError("El campo IMPORTES_AJUSTES    tiene el registro: " + rta[3].toString() + ". Se esperaba: 0");
			System.out.println("El campo IMPORTES_AJUSTES    tiene el registro: " + rta[3].toString() + ". Se esperaba: 0\r\n");
			Status[3] = "FAIL - Validación del IMPORTES_AJUSTES   ";
		}	else {
			report.AddLine("Resultado Correcto. El campo IMPORTES_AJUSTES    tiene el registro: " + rta[3].toString());
			System.out.println("Resultado Correcto. El campo IMPORTES_AJUSTES    tiene el registro: " + rta[3] + "\r\n");
			Status[3] = "P";
		}
	...
	private boolean execPreCondiciones(Reports report, dbWorker oraWorker) {
		...
		//DELETE
		queryPre = "delete from control_liq_comercios clc where clc.id_comercio = '8228' and clc.fecha_cierre = '21/07/2022' and clc.fecha_pago = '26/07/2022' and clc.id_moneda = '858' and clc.id_marca = '2'";
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//DELETE
		queryPost = "delete from control_liq_comercios clc where clc.id_comercio = '8228' and clc.fecha_cierre = '14/07/2022 ' and clc.fecha_pago = '19/07/2022 ' and clc.id_moneda = '858' and clc.id_marca = '2'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 01		
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '21413'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 02	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021413'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021413'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '021413'))";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '020991'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '020991'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '020991'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '020991'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '020991'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '020991'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '020991'", conn);
