...
import Pasos.GlobalBatch_ADQ_701.TC_45_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_45_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_45_GB_PASOS_ADQ_701 pasos = new TC_45_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_45_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif ="select cc.nro_cuota, cc.fecha_pago_comercio "
				...
				+ "where a.id_autorizacion_adquirente in ('021415') and cc.nro_cuota = '1'";
		...
		queryVerif ="select cc.nro_cuota, cc.fecha_pago_comercio "
				...
				+ "inner join autorizacion a on c.id_autorizacion = a.id_autorizacion "
				+ "inner join autorizacion_adquirente_log aal on a.id_autorizacion_adquirente = aal.id_autorizacion_adquirente "
				+ "where a.id_autorizacion_adquirente in ('021415') and cc.nro_cuota = '2'";
		...
		queryVerif ="select cc.nro_cuota, cc.fecha_pago_comercio "
				...
				+ "inner join autorizacion a on c.id_autorizacion = a.id_autorizacion "
				+ "inner join autorizacion_adquirente_log aal on a.id_autorizacion_adquirente = aal.id_autorizacion_adquirente "
				+ "where a.id_autorizacion_adquirente in ('021415') and cc.nro_cuota = '3'";
	...
	private void primeraValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Validación del NRO_CUOTA - Tiene que ser igual a 1
		if (!respVerifBDD[0].equals("1")) {
			report.AddLineAssertionError("El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString() + ". Se esperaba: 1");
			System.out.println("El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString() + ". Se esperaba: 1\r\n");
			Status[0] = "FAIL - Validación del NRO_CUOTA";
		}	else {
			report.AddLine("Resultado Correcto. El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString());
			System.out.println("Resultado Correcto. El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0] + "\r\n");
			Status[0] = "P";
		}

		//Validación de FECHA_PAGO_COMERCIO - Tiene que ser igual a 2022-08-22 00:00:00
		if (!respVerifBDD[1].equals("2022-08-22 00:00:00")) {
			report.AddLineAssertionError("El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + ". Se esperaba: 2022-08-22 00:00:00");
			System.out.println("El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + ". Se esperaba: 2022-08-22 00:00:00\r\n");
			Status[1] = "FAIL - Validación del FECHA_PAGO_COMERCIO";
		}	else {
			report.AddLine("Resultado Correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1]);
			System.out.println("Resultado Correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + "\r\n");
			Status[1] = "P";
		}	
	}
	...
	private void segundaValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Validación del NRO_CUOTA - Tiene que ser igual a 2
		if (!respVerifBDD[0].equals("2")) {
			report.AddLineAssertionError("El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString() + ". Se esperaba: 2");
			System.out.println("El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString() + ". Se esperaba: 2\r\n");
			Status[2] = "FAIL - Validación del NRO_CUOTA";
		}	else {
			report.AddLine("Resultado Correcto. El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString());
			System.out.println("Resultado Correcto. El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0] + "\r\n");
			Status[2] = "P";
		...
		//Validación de FECHA_PAGO_COMERCIO - Tiene que ser igual a 2022-09-19 00:00:00
		if (!respVerifBDD[1].equals("2022-09-19 00:00:00")) {
			report.AddLineAssertionError("El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + ". Se esperaba: 2022-09-19 00:00:00");
			System.out.println("El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + ". Se esperaba: 2022-09-19 00:00:00\r\n");
			Status[3] = "FAIL - Validación del FECHA_PAGO_COMERCIO";
		}	else {
			report.AddLine("Resultado Correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1]);
			System.out.println("Resultado Correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + "\r\n");
			Status[3] = "P";
	...
	private void terceraValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Validación del NRO_CUOTA - Tiene que ser igual a 3
		if (!respVerifBDD[0].equals("3")) {
			report.AddLineAssertionError("El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString() + ". Se esperaba: 3");
			System.out.println("El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString() + ". Se esperaba: 3\r\n");
			Status[4] = "FAIL - Validación del NRO_CUOTA";
		}	else {
			report.AddLine("Resultado Correcto. El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0].toString());
			System.out.println("Resultado Correcto. El campo NRO_CUOTA tiene el registro: " + respVerifBDD[0] + "\r\n");
			Status[4] = "P";
		...
		//Validación de FECHA_PAGO_COMERCIO - Tiene que ser igual a 2022-10-17 00:00:00
		if (!respVerifBDD[1].equals("2022-10-17 00:00:00")) {
			report.AddLineAssertionError("El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + ". Se esperaba: 2022-10-17 00:00:00");
			System.out.println("El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + ". Se esperaba: 2022-10-17 00:00:00\r\n");
			Status[5] = "FAIL - Validación del FECHA_PAGO_COMERCIO";
		}	else {
			report.AddLine("Resultado Correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1]);
			System.out.println("Resultado Correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: " + respVerifBDD[1] + "\r\n");
			Status[5] = "P";
