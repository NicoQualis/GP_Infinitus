...
import Pasos.GlobalBatch_ADQ_701.TC_39_GB_PASOS_ADQ_701;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_39_WEB_GB_701 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		TC_39_GB_PASOS_ADQ_701 pasos = new TC_39_GB_PASOS_ADQ_701();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_39_GB_PASOS_ADQ_701 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif ="select c.id_mod_financ_comercio, c.importe_interes_com, c.porcentaje_financ, c.id_sistema_financiacion, c.tipo_porc_financ, c.importe_iva_interes_com "
				...
				+ "where a.id_autorizacion_adquirente in ('21546')";
		...
		//Continua a pesar que el valor de la tabla sea null
			...
			if (rta[0].equals("12")) {
				...
				report.AddLineAssertionError("FAIL - La tabla ID_MOD_FINANC_COMERCIO tiene el registro: "+rta[0]+", se esperaba el registro 12");
				System.out.println("FAIL - La tabla ID_MOD_FINANC_COMERCIO  tiene el registro: "+rta[0]+", se esperaba el registro 12");
			...
			if (rta[1].equals("-434.05")) {
				...
				report.AddLineAssertionError("FAIL - La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -434.05");
				System.out.println("FAIL - La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -434.05");
			...
			if (rta[2].equals("10")) {
				...
				System.out.println("FAIL - La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]+", se esperaba el registro 10");
			...
			if (rta[3].equals("2")) {
				...
				report.AddLineAssertionError("ERR - La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]+", se esperaba el registro 2");
				System.out.println("FAIL - La tabla ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]+", se esperaba el registro 2");
			...
			if (rta[4].equals("2")) {
				...
				report.AddLineAssertionError("FAIL- La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]+", se esperaba el registro 2");
				System.out.println("FAIL - La tabla TIPO_PORC_FINANC tiene el registro: "+rta[4]+", se esperaba el registro 2");
			...
			if (rta[5].equals("-95.49")) {
				...
				report.AddLineAssertionError("FAIL - La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -95.49");
				System.out.println("FAIL - La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -95.49");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 01		
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '21546'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 02	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21546'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21546'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21546'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '21546'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 06	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21546'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 07	
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21546'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '21546'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21546'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21546'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '21546'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '21546'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21546'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '21546'", conn);
