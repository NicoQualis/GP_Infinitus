...
import Pasos.GlobalBatch_ADQ_700.TC_74_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_74_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_74_GB_PASOS_ADQ_700 pasos = new TC_74_GB_PASOS_ADQ_700();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_74_GB_PASOS_ADQ_700 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		//tercera validación
		queryVerif ="select c.fecha_pago, c.nro_liquidacion from comercios_liq_cabecera c where c.id_comercio_central = '8228' and c.fecha_cierre_liquidacion = '04/08/2022' order by c.nro_liquidacion desc";
	...
	private void primeraValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Validación del LIQ_COMERCIO   - Tiene que ser igual a 1
	...
	private void terceraValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Guarda en variable el Nro_Liquidacion para la cuarta validacion
		NRO_LIQUIDACION = respVerifBDD[1];

		//Validación del FECHA_PAGO    - Tiene que ser igual a 2022-08-09 00:00:00
		if (!respVerifBDD[0].equals("2022-08-09 00:00:00")) {
			report.AddLineAssertionError("Resultado no esperado de la validacion 3. El campo FECHA_PAGO   tiene el registro :" + respVerifBDD[0].toString() + " Se esperaba: 2022-08-09 00:00:00");
			System.out.println("Resultado no esperado de la validacion 3. El campo FECHA_PAGO   tiene el registro :" + respVerifBDD[0].toString() + " Se esperaba: 2022-08-09 00:00:00 \r\n");
			Status[2] = "FAIL - Validación del FECHA_PAGO  ";
		}	else {
			report.AddLine("Resultado correcto de la validacion 3. El campo FECHA_PAGO   tiene el registro: " + respVerifBDD[0].toString());
			System.out.println("Resultado Correcto correcto de la validacion 3. El campo FECHA_PAGO   tiene el registro: " + respVerifBDD[0].toString() + "\r\n");
			Status[2] = "P";
		}
	...
	private void cuartaValidacion(dbWorker oraResp, Reports report, String query) throws SQLException {
		...
		//Validación del ID_CONSUMO   - Tiene que ser igual a NRO_LIQUDACION de validacion 3
		if (!respVerifBDD.equals("473647")) {
			report.AddLineAssertionError("Resultado no esperado de la validacion 4. El campo ID_CONSUMO  tiene el registro :" + respVerifBDD.toString() + " Se esperaba: 473647 ");
			System.out.println("Resultado no esperado de la validacion 4. El campo ID_CONSUMO  tiene el registro :" + respVerifBDD.toString() + " Se esperaba: 473647 ");
			Status[3] = "FAIL - Validación del ID_CONSUMO ";
		}	else {
			report.AddLine("Resultado correcto de la validacion 4. El campo ID_CONSUMO  tiene el registro: " + respVerifBDD.toString());
			System.out.println("Resultado correcto de la validacion 4. El campo ID_CONSUMO  tiene el registro: " + respVerifBDD.toString() + "\r\n");
			Status[3] = "P";
		}
	...
	private boolean execPreCondiciones(Reports report, dbWorker oraWorker) {
		...
		//Acción 07-
		queryPre = "Insert into CONSUMOS_CUOTAS (ID_CONSUMO,NRO_CUOTA,CUOTAS,IMPORTE_CUOTA,IMPORTE_INTERES,IMPORTE_IVA_INTERES,LIQ_SOCIO,LIQ_COMERCIO,CAIDA_CUOTAS,FIDEICOMISO,CONTRAPARTIDO,ID_AUDITORIA,ROW_VERSION,ROW_FECHA,ID_ESTADO_TIPO_CONFIRM,ID_ESTADO_CONFIRM,FECHA_CIERRE_COMERCIO,FECHA_PAGO_COMERCIO,FECHA_CIERRE_SOCIO,FECHA_IPM,ARANCEL,IMPORTE_ARANCEL) values ('473647','1','1','30000','0','0','0','0','0','0','0','19000','3',to_date('10/08/2022 18:12:48','DD/MM/RRRR HH24:MI:SS'),'40','1',to_date('04/08/2022 00:00:00','DD/MM/RRRR HH24:MI:SS'),to_date('09/08/2022 00:00:00','DD/MM/RRRR HH24:MI:SS'),null,to_date('04/08/2022 20:53:34','DD/MM/RRRR HH24:MI:SS'),'50','-15000')";
		res.add(oraWorker.oraInsert(queryPre, conn));

		//DELETES
		//Acción 08-
		queryPre = "update entidades set tipo_fecha_liq_comercio = 'P' where id_entidad = '700'";
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Deletes referidas al proceso de liquidacion
		...
		//Accion 08		
		queryPost = "delete ctf_visa where id_consumo = '473647'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		...
		//Update
		//Accion 16		
		queryPost = "update entidades set tipo_fecha_liq_comercio = 'C' where id_entidad = '700'";
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		//Deletes referidas al consumo
		oraWorker.oraDelete("delete ctf_visa where id_consumo = '473647'", conn);
		oraWorker.oraDelete("delete from consumos_cuotas where id_consumo = '473647'", conn);
		oraWorker.oraDelete("delete from consumos_datos_adicionales where id_consumo = '473647'", conn);
		oraWorker.oraDelete("delete from presentaciones where id_consumo = '473647'", conn);
		oraWorker.oraDelete("delete from consumos where id_consumo = '473647'", conn);
		oraWorker.oraDelete("delete from autorizacion where id_autorizacion_adquirente = '350211'", conn);
		oraWorker.oraDelete("delete from respuesta_mc_log where id_autorizacion_adquirente = '350211'", conn);
		oraWorker.oraDelete("delete from autorizacion_adquirente_log where id_autorizacion_adquirente = '350211'", conn);
		//Update referidas a entidades
		oraWorker.oraUpdate("update entidades set tipo_fecha_liq_comercio = 'C' where id_entidad = '700'", conn);	
