...
import Pasos.GlobalBatch_ADQ_700.TC_39_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_39_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		TC_39_GB_PASOS_ADQ_700 pasos = new TC_39_GB_PASOS_ADQ_700();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_39_GB_PASOS_ADQ_700 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif = "select c.id_mod_financ_comercio, c.importe_interes_com, c.porcentaje_financ, c.id_sistema_financiacion, c.tipo_porc_financ, c.importe_iva_interes_com "
				...
				+ "where a.id_autorizacion_adquirente in ('348518')";
		...
		if (rta[0].equals("15")) {
			...
			report.AddLineAssertionError("El campo id_mod_financ_comercio tiene el registro:" + rta[0] + ", Se esperaba el registro 15");
			System.out.println("El campo id_mod_financ_comercio tiene el registro:" + rta[0] + ", Se esperaba el registro 15");
		...
		if (rta[1].equals("-839.43")) {
			...
			report.AddLineAssertionError("El campo importe_interes_com tiene el registro: "+rta[1]+", se esperaba el registro -839,43");
			System.out.println("El campo importe_interes_com tiene el registro: "+rta[1]+", se esperaba el registro -839,43");
		...
		if (rta[2].equals("20")) {
			...
			report.AddLineAssertionError("El campo porcentaje_financ tiene el registro: "+rta[2]+", se esperaba el registro 20");
			System.out.println("El campo porcentaje_financ tiene el registro: "+rta[2]+", se esperaba el registro 20");
			Status[2] = "FAIL - porcentaje_financ";
		}		
		
		//Validacion 04
		if (rta[3].contentEquals("2")) {
			...
			report.AddLineAssertionError("El campo id_sistema_financiacion tiene el registro: "+rta[3]+", se esperaba el registro 2");
			System.out.println("El campo id_sistema_financiacion tiene el registro: "+rta[3]+", se esperaba el registro 2");
			Status[3] = "FAIL - id_sistema_financiacion";
		}
		
		//Validacion 05		
		if (rta[4].contentEquals("2")) {
			...
			report.AddLineAssertionError("El campo tipo_porc_financ tiene el registro: "+rta[4]+", se esperaba el registro 2");
			System.out.println("El campo tipo_porc_financ tiene el registro: "+rta[4]+", se esperaba el registro 2");
		...
		if (rta[5].equals("-176.28")) {
			...
			report.AddLineAssertionError("El campo importe_iva_interes_com tiene el registro: "+rta[5]+", se esperaba el registro -176,28");
			System.out.println("El campo importe_iva_interes_com tiene el registro: "+rta[5]+", se esperaba el registro -176,28");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		queryPost = "DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348518')";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 02	- TABLA CONSUMOS_CUOTAS
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348518')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	- TABLA CONSUMOS_DATOS_ADICIONALES
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348518')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	- TABLA CONSUMOS
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348518')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	- TABLA AUTORIZACION
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348518'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 06	- TABLA RESPUESTA_MC_LOG
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348518'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	
		//Accion 07	- TABLA AUTORIZACION_ADQUIRENTE_LOG
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348518'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348518')", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348518')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348518')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348518')))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348518'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348518'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348518'", conn);
