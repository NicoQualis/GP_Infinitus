...
import Pasos.GlobalBatch_ADQ_700.TC_13_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_13_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_13_GB_PASOS_ADQ_700 pasos = new TC_13_GB_PASOS_ADQ_700();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_13_GB_PASOS_ADQ_700 pasos) {
...
;
		...
		//Validacion 01
		queryVerif ="select c.tipo_producto, c.fecha_pago_comercio\r\n"
				+ "from consumos c\r\n"
				...
				+ "where a.id_autorizacion_adquirente = '350221'\r\n"
				+ "group by c.tipo_producto, c.fecha_pago_comercio";
		rta = oraWorker.oraTwoQuery(queryVerif);

		if (rta[0].equals("C")) {
			report.AddLine("Resultado correcto. El campo TIPO_PRODUCTO tiene el registro: "+rta[0]);
			System.out.println("Resultado correcto. El campo TIPO_PRODUCTO tiene el registro: "+rta[0]+"\r\n");
			Status[0] = "P";
		} else {
			report.AddLineAssertionError("El campo TIPO_PRODUCTO tiene el registro: "+rta[0]+", se esperaba el registro C");
			System.out.println("El campo TIPO_PRODUCTO tiene el registro: "+rta[0]+", se esperaba el registro C \r\n");
			Status[0] = "FAIL - TIPO_PRODUCTO";
		}

		//Validacion 02
		if (rta[1].equals("2022-08-16 00:00:00")) {
			report.AddLine("Resultado correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: "+rta[1]);
			System.out.println("Resultado correcto. El campo FECHA_PAGO_COMERCIO tiene el registro: "+rta[1]+"\r\n");
			Status[1] = "P";
		} else {
			report.AddLineAssertionError("El campo FECHA_PAGO_COMERCIO tiene el registro: "+rta[1]+", se esperaba el registro 2022-08-16 00:00:00");
			System.out.println("El campo FECHA_PAGO_COMERCIO tiene el registro: "+rta[1]+", se esperaba el registro 2022-08-16 00:00:00 \r\n");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 01		
		queryPost = "DELETE FROM FERIADOS WHERE DESCRIPCION = 'Feriado Automation'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 02	
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '350221'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '350221'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '350221'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '350221'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 06	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '350221'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 07
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '350221'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 08
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '350221'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM FERIADOS WHERE DESCRIPCION = 'Feriado Automation'", conn);
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '350221'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '350221'))"	, conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '350221'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '350221'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '350221'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '350221'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '350221'", conn);
