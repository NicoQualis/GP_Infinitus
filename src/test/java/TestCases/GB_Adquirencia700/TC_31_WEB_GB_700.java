...
import Pasos.GlobalBatch_ADQ_700.TC_31_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_31_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		TC_31_GB_PASOS_ADQ_700 pasos = new TC_31_GB_PASOS_ADQ_700();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_31_GB_PASOS_ADQ_700 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif = "select c.id_mod_financ_comercio, c.porcentaje_financ, c.id_sistema_financiacion, c.tipo_porc_financ  "
				+ "from consumos c "
				...
				+ "where a.id_autorizacion_adquirente = '348022'";
		...
		if (rta[0]==null) {
			...
			report.AddLineAssertionError("El campo id_mod_financ_comercio tiene el registro:" + rta[0] + ", Se esperaba el registro 'null'");
			System.out.println("El campo id_mod_financ_comercio tiene el registro:" + rta[0] + ", Se esperaba el registro 'null'");
			Status[0] = "FAIL - id_mod_financ_comercio";
		}
		
		//Validacion 02
		if (rta[1]==null) {
			report.AddLine("Resultado Correcto. El campo porcentaje_financ tiene el registro: " + rta[1]);
			System.out.println("Resultado Correcto. El campo porcentaje_financ tiene el registro: " + rta[1]);
			Status[1] = "P";
		} else {
			report.AddLineAssertionError("El campo porcentaje_financ tiene el registro: "+rta[1]+", se esperaba el registro null");
			System.out.println("El campo porcentaje_financ tiene el registro: "+rta[1]+", se esperaba el registro null");
			Status[1] = "FAIL - porcentaje_financ";
		}		
		
		//Validacion 03
		if (rta[2]==null) {
			report.AddLine("Resultado Correcto. El campo id_sistema_financiacion tiene el registro: " + rta[2]);
			System.out.println("Resultado Correcto. El campo id_sistema_financiacion tiene el registro: " + rta[2]);
			Status[2] = "P";
		} else {
			report.AddLineAssertionError("El campo id_sistema_financiacion tiene el registro: "+rta[2]+", se esperaba el registro null");
			System.out.println("El campo id_sistema_financiacion tiene el registro: "+rta[2]+", se esperaba el registro null");
			Status[2] = "FAIL - id_sistema_financiacion";
		}
		
		//Validacion 04		
		if (rta[3]==null) {
			report.AddLine("Resultado Correcto. El campo tipo_porc_financ tiene el registro: " + rta[3]);
			System.out.println("Resultado Correcto. El campo tipo_porc_financ tiene el registro: " + rta[3]);
			Status[3] = "P";
		} else {
			report.AddLineAssertionError("El campo tipo_porc_financ tiene el registro: "+rta[3]+", se esperaba el registro null");
			System.out.println("El campo tipo_porc_financ tiene el registro: "+rta[3]+", se esperaba el registro null");
			Status[3] = "FAIL - tipo_porc_financ";
		}
		
		queryVerif ="select c.importe_interes_com "
				+ "from consumos c "
				+ "inner join consumos_datos_adicionales cda on c.id_consumo = cda.id_consumo "
				+ "inner join consumos_cuotas cc on c.id_consumo = cc.id_consumo "
				+ "inner join presentaciones p on c.id_consumo = p.id_consumo "
		...
		rta2 = oraWorker.oraOneQuery(queryVerif);
		queryVerif = "";

		//Validacion 05
		if (rta2.equals("0")) {
			report.AddLine("Resultado Correcto. El campo importe_interes_com tiene el registro: " + rta2+".");
			System.out.println("Resultado Correcto. El campo importe_interes_com tiene el registro: " + rta2+".");
			Status[4] = "P";
		} else {
			report.AddLineAssertionError("El campo importe_interes_com tiene el registro: "+rta2+", se esperaba el registro 0");
			System.out.println("El campo importe_interes_com tiene el registro: "+rta2+", se esperaba el registro 0");
			Status[4] = "FAIL - importe_interes_com";
		}
		
		queryVerif ="select c.importe_iva_interes_com "
				+ "from consumos c "
				+ "inner join consumos_datos_adicionales cda on c.id_consumo = cda.id_consumo "
				+ "inner join consumos_cuotas cc on c.id_consumo = cc.id_consumo "
				+ "inner join presentaciones p on c.id_consumo = p.id_consumo "
		...
		//Validacion 06		
		if (rta2.equals("0")) {
			report.AddLine("Resultado Correcto. El campo importe_iva_interes_com tiene el registro: " + rta2+".");
			System.out.println("Resultado Correcto. El campo importe_iva_interes_com tiene el registro: " + rta2+".");
			Status[5] = "P";
		} else {
			report.AddLineAssertionError("El campo importe_iva_interes_com tiene el registro: "+rta2+", se esperaba el registro 0");
			System.out.println("El campo importe_iva_interes_com tiene el registro: "+rta2+", se esperaba el registro 0");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 01		
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '348022'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 02	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '348022'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '348022'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '348022'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348022'";
		res.add(oraWorker.oraDelete(queryPost, conn));
		
		//Accion 06	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348022'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	
		//Accion 07	
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348022'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '348022'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '348022'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '348022'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '348022'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348022'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348022'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348022'", conn);
