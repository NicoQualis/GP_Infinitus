...
import Pasos.GlobalBatch_ADQ_700.TC_42_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_42_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_42_GB_PASOS_ADQ_700 pasos = new TC_42_GB_PASOS_ADQ_700();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_42_GB_PASOS_ADQ_700 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif ="select c.id_mod_financ_comercio, c.importe_interes_com, c.porcentaje_financ, c.id_sistema_financiacion, c.tipo_porc_financ, c.importe_iva_interes_com\r\n"
				...
				+ "where a.id_autorizacion_adquirente in ('348521')";
		...
		if (rta[0].equals("18")) {
			...
			report.AddLineAssertionError("ERR - La tabla ID_MOD_FINANC_COMERCIO tiene el registro: "+rta[0]+", se esperaba el registro 18");
			System.out.println("ERR - La tabla ID_MOD_FINANC_COMERCIO  tiene el registro: "+rta[0]+", se esperaba el registro 18");
			Status[0] = "FAIL - ID_MOD_FINANC_COMERCIO";
		}

		//Validacion 02 - IMPORTE_INTERES_COM
		if (rta[1].equals("-839.43")) {
			...
			report.AddLineAssertionError("ERR - La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -839.43");
			System.out.println("ERR - La tabla IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -839.43");
		...
		if (rta[2].equals("20")) {
			...
			report.AddLineAssertionError("ERR - La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]+", se esperaba el registro 20");
			System.out.println("ERR - La tabla PORCENTAJE_FINANC tiene el registro: "+rta[2]+", se esperaba el registro 20");
		...
		if (rta[5].equals("-176.28")) {
			...
			report.AddLineAssertionError("ERR - La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -176.28");
			System.out.println("ERR - La tabla IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -176.28");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 01		
		queryPost = "DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348521')";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 02	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348521')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348521')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348521')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348521'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 06	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348521'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 07	
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348521'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348521')", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348521')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348521')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348521')))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348521'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348521'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348521'", conn);
