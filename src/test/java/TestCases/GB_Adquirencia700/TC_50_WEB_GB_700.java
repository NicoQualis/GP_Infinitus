...
import Pasos.GlobalBatch_ADQ_700.TC_50_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_50_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_50_GB_PASOS_ADQ_700 pasos = new TC_50_GB_PASOS_ADQ_700();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_50_GB_PASOS_ADQ_700 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif ="select clc.cant_consumos, clc.importes_consumos, clc.cant_ajustes, clc.importes_ajustes \r\n"
				...
				+ "and clc.fecha_cierre = '20/07/2022' \r\n"
				+ "and clc.fecha_pago = '28/07/2022' \r\n"
				+ "and clc.id_moneda = '32'  \r\n"
				+ "and clc.id_marca = '2'";
		...
		queryVerif ="select clc.cant_consumos, clc.importes_consumos, clc.cant_ajustes, clc.importes_ajustes \r\n"
				+ "from control_liq_comercios clc \r\n"
				+ "where clc.id_comercio = '8248' \r\n"
				+ "and clc.fecha_cierre = '17/08/2022' \r\n"
				+ "and clc.fecha_pago = '25/08/2022' \r\n"
				+ "and clc.id_moneda = '32'  \r\n"
				+ "and clc.id_marca = '2'";
		...
		queryVerif ="select clc.cant_consumos, clc.importes_consumos, clc.cant_ajustes, clc.importes_ajustes \r\n"
				+ "from control_liq_comercios clc \r\n"
				+ "where clc.id_comercio = '8248' \r\n"
				+ "and clc.fecha_cierre = '14/09/2022' \r\n"
				+ "and clc.fecha_pago = '22/09/2022' \r\n"
				+ "and clc.id_moneda = '32' \r\n"
				+ "and clc.id_marca = '2'";
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 02	
		queryPost = "DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348411')";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348411')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348411')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348411')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 06	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348411'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 07	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348411'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 08	
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348411'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348411')", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348411')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348411')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348411')))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348411'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348411'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348411'", conn);
