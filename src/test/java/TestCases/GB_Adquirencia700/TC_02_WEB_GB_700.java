...
import Pasos.GlobalBatch_ADQ_700.TC_02_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_02_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		TC_02_GB_PASOS_ADQ_700 pasos = new TC_02_GB_PASOS_ADQ_700();
	...
	private boolean execPreCondiciones(Reports report, dbWorker oraWorker) {
		...
		//Acción 02 - AUTORIZACION_ADQUIRENTE_LOG
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_02_GB_PASOS_ADQ_700 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		//Validacion 01
		queryVerif = "select a.fecha_relacion "
				+ "from consumos c "
				...
				+ "where a.id_autorizacion_adquirente = '344876'";
		rta = oraWorker.oraOneQuery(queryVerif);
		
		if (rta.isEmpty()==false) {
			report.AddLine("Resultado Correcto. El campo fecha_relacion tiene el registro: " + rta);
			System.out.println("Resultado Correcto. El campo fecha_relacion tiene el registro: " + rta);
			Status[0] = "P";
		} else {
			report.AddLineAssertionError("El campo fecha_relacion tiene el registro: " + rta + ". Se esperaba NOT NULL");
			System.out.println("El campo fecha_relacion tiene el registro: " + rta + ". Se esperaba NOT NULL");
			Status[0] = "FAIL - FECHA_RELACION";
		}


		//Validación 02
		queryVerif = "select aal.presentacion_procesada "
				+ "from consumos c "
				+ "inner join consumos_datos_adicionales cda on c.id_consumo = cda.id_consumo "
				+ "inner join consumos_cuotas cc on c.id_consumo = cc.id_consumo "
				+ "inner join presentaciones p on c.id_consumo = p.id_consumo "
		...
		if (rta.equals("1")) {
			report.AddLine("Resultado Correcto. El campo presentacion_procesada tiene el registro: " + rta + ".");
			System.out.println("Resultado Correcto. El campo presentacion_procesada tiene el registro: " + rta + ".");
			Status[1] = "P";
		} else {
			report.AddLineAssertionError("El campo presentacion_procesada tiene el registro: " + rta + ". Se esperaba 1");
			System.out.println("El campo presentacion_procesada tiene el registro: " + rta + ". Se esperaba 1");
			Status[1] = "FAIL - PRESENTACION_PROCESADA";
		}
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		queryPost = "DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '344876'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 02 - TABLA CONSUMOS_CUOTAS
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '344876'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03 - TABLA CONSUMOS_DATOS_ADICIONALES
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '344876'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04 - TABLA ID_CONSUMO
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '344876'))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05 - TABLA ID_AUTORIZACION_ADQUIRENTE
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '344876'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 06 - TABLA RESPUESTA_MC_LOG
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '344876'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 07 - TABLA AUTORIZACION_ADQUIRENTE_LOG
				queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '344876'";
				res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES WHERE id_autorizacion_adquirente = '344876'", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '344876'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '344876'))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente = '344876'))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '344876'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '344876'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '344876'", conn);
