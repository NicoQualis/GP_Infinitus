...
import Pasos.GlobalBatch_ADQ_700.TC_38_GB_PASOS_ADQ_700;
import Repositories.Repo_WebRepository;
import Tools.dbWorker;
import io.restassured.path.json.JsonPath;

public class TC_38_WEB_GB_700 {
	...
	public boolean Test(Datasources data, Reports report, DriverManager DM, int iteration, String name, String configEntidad) {
		...
		try {
			...
			TC_38_GB_PASOS_ADQ_700 pasos = new TC_38_GB_PASOS_ADQ_700();
	...
	private boolean validacionesWeb(Reports report, Repo_WebRepository repo, DriverManager DM, String name, String runId, TC_38_GB_PASOS_ADQ_700 pasos) {
	...
	private boolean validacionesBD (dbWorker oraWorker, Reports report) throws SQLException {
		...
		queryVerif = "select c.id_mod_financ_comercio, c.importe_interes_com, c.porcentaje_financ, c.id_sistema_financiacion, c.tipo_porc_financ, c.importe_iva_interes_com "
				...
				+ "where a.id_autorizacion_adquirente in ('348508')";
		...
		if (rta[0].equals("14")) { 
			...
			report.AddLineAssertionError("El campo ID_MOD_FINANC_COMERCIO tiene el registro: "+rta[0]+", se esperaba el registro 14");
			System.out.println("El campo ID_MOD_FINANC_COMERCIO  tiene el registro: "+rta[0]+", se esperaba el registro 14 \r\n");
			Status[0] = "FAIL - ID_MOD_FINANC_COMERCIO";
		}

		// Validacion 02
		if (rta[1].equals("-1061.51")) { 
			...
			report.AddLineAssertionError("El campo IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -1061.51");
			System.out.println("El campo IMPORTE_INTERES_COM tiene el registro: "+rta[1]+", se esperaba el registro -1061.51 \r\n");
		...
		if (rta[3].equals("1")) { 
			...
			report.AddLineAssertionError("El campo ID_SISTEMA_FINANCIACION tiene el registro: "+rta[3]+", se esperaba el registro 1");
			System.out.println("el campo ID_SISTEMA_FINANCIACION  tiene el registro: "+rta[3]+", se esperaba el registro 1 \r\n");
		...
		if (rta[5].equals("-222.92")) { 
			...
			report.AddLineAssertionError("El campo IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -222.92");
			System.out.println("El campo IMPORTE_IVA_INTERES_COM tiene el registro: "+rta[5]+", se esperaba el registro -222.92 \r\n");
	...
	private boolean execPostCondiciones (Reports report, dbWorker oraWorker) {
		...
		//Accion 01		
		queryPost = "DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348508')";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 02	
		queryPost = "DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348508')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 03	
		queryPost = "DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348508')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 04	
		queryPost = "DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348508')))";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 05	
		queryPost = "DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348508'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 06	
		queryPost = "DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348508'";
		res.add(oraWorker.oraDelete(queryPost, conn));

		//Accion 07	
		queryPost = "DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348508'";
		res.add(oraWorker.oraDelete(queryPost, conn));
	...
	private void execPrePostCondi(Reports report, dbWorker oraWorker) {
		...
		oraWorker.oraDelete("DELETE FROM PRESENTACIONES where id_autorizacion_adquirente in ('348508')", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_CUOTAS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348508')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS_DATOS_ADICIONALES WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348508')))", conn);
		oraWorker.oraDelete("DELETE FROM CONSUMOS WHERE ID_CONSUMO in (select id_consumo from consumos where id_autorizacion in (select id_autorizacion from autorizacion where id_autorizacion_adquirente in ('348508')))", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION WHERE ID_AUTORIZACION_ADQUIRENTE = '348508'", conn);
		oraWorker.oraDelete("DELETE FROM RESPUESTA_MC_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348508'", conn);
		oraWorker.oraDelete("DELETE FROM AUTORIZACION_ADQUIRENTE_LOG WHERE ID_AUTORIZACION_ADQUIRENTE = '348508'", conn);
